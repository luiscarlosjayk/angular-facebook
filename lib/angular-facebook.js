  ...
  angular.module('facebook', []).
    ...
    run([
      ...
      function($rootScope, $q, $window, $timeout) {
        ...
        var fbDivDeferred = $q.defer();
        var facebookAppDeferred = $q.defer();

        loadDeferred = fbDivDeferred.promise.then(function(){
          return facebookAppDeferred.promise;
        },function(error){
          var defer = $q.defer();
          defer.reject(error);
        });
        ...
        $window.fbAsyncInit = function() {
          ...
          $timeout(function() {
            ...
            facebookAppDeferred.resolve(FB);
          });
        };
        ...
        var loadFBElement = function() {
          fbroot = document.createElement('div');
          fbroot.id = 'fb-root';
          document.body.insertBefore(fbroot, document.body.childNodes[0]);
          fbDivDeferred.resolve(fbroot);
          return fbroot;
        };

        if (!fbroot) {
          if (angular.isDefined(document.body)) {
            loadFBElement();
          } else {
            addLoadEvent(loadFBElement);
          }
        }
